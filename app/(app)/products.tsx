...
import { Card, EmptyText, IconChip, Screen, SectionHeader } from '../../src/components/ui';
...
export default function ProductsScreen() {
  ...
  return (
    <Screen fab={<Fab label="Thêm" onPress={() => router.push('/(app)/product-form' as never)} />}>
      ...
      {db.products.length === 0 ? <EmptyText icon="package">Chưa có sản phẩm nào</EmptyText> : (
        <Card padded={false}>
          {db.products.map((p, i) => {
            ...
            return (
              <TouchableOpacity key={p.sku} style={[s.row, i < db.products.length - 1 && s.rowBorder]} activeOpacity={0.7} onPress={() => router.push({ pathname: '/(app)/product-form', params: { sku: p.sku } } as never)}>
