...
import { whShort } from '../../src/utils/labels';
...
export default function UsersScreen() {
  ...
  return (
    <Screen fab={<Fab label="Thêm" onPress={() => router.push('/(app)/user-form' as never)} />}>
      ...
      {db.users.length === 0 ? <EmptyText icon="users">Chưa có tài khoản nào</EmptyText> : (
        <Card padded={false}>
          {db.users.map((u, i) => {
            ...
            return (
              <TouchableOpacity key={u.id} style={[s.row, i < db.users.length - 1 && s.rowBorder]} activeOpacity={0.7} onPress={() => router.push({ pathname: '/(app)/user-form', params: { id: u.id } } as never)}>
